...
def main(A, B, C, eps, sim_steps = 100):
    ...
    for ii in range(0, steps):
        ...
        # print ("prob_array[-1] = ", prob_array[-1])
        ...
        index = 0
        while (index < len(pop)):
            # random choice on the roullette from
            choice = np.random.uniform(0.0, prob_array[-1])
            # print ("choice = ", choice)
            for i in range(pop_size):
...
if __name__ == "__main__":
    # main(0.5, 1.0, 0.1, 10**-2, 100)
    mainR(0.5, 1.0, 0.1, 10**-1, 100)
